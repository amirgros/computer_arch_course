                   ...
                   unsigned L1Cyc, unsigned L2Cyc, unsigned WrAlloc)
{
    ...
    for (unsigned i = 0; i < L1.Ways.size(); i++)
    {
        L1.Ways[i].resize(1 << (L1Size - BSize - L1NumWays));
        for (unsigned j = 0; j < L1.Ways[i].size(); j++)
        {
    ...
    for (unsigned i = 0; i < L2.Ways.size(); i++)
    {
        L2.Ways[i].resize(1 << (L2Size - BSize - L2NumWays));
        for (unsigned j = 0; j < L2.Ways[i].size(); j++)
        {
...
{
    unsigned tmp = c.Ways[way_index][set_index].counter;
    c.Ways[way_index][set_index].counter = c.Ways.size() - 1;
    for (unsigned i = 0; i < c.Ways.size(); i++)
    {
